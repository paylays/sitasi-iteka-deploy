            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs-custom card-header-tabs border-top mt-4" id="pills-tab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link px-3 active" href="{{ route('data-pengajuan:seminar-proposal') }}" >Mahasiswa Bimbingan</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link px-3" href="{{ route('data-pengajuan:seminar-proposal', ['type' => 'mahasiswa-uji']) }}" >Mahasiswa Uji</a>
                        </li>
                    </ul>
                </div>
                <!-- end card body -->
            ...
            <div class="card card-body blur shadow-blur">
                <div class="row align-items-center">
                    <div class="card-header">
                        <h2 class="my-heading">Daftar Mahasiswa Seminar Proposal</h2>
                    </div>
                    <div class="card-body">
                                ...
                                <table class="table align-middle datatable dt-responsive table-check nowrap" style="border-collapse: collapse; border-spacing: 0 8px; width: 100%;">
                                    <thead>
                                      <tr>
                                        ...
                                        <th scope="col" class="text-center">Persetujuan Sempro</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td wire:click="openDetail('{{ $pengajuan->id }}')">
                                                @if($pengajuan->mahasiswa->user->sempro && $pengajuan->mahasiswa->user->sempro->penilaianSempros()->first())
                                                <span class="badge bg-success" style="font-size: 10pt;">Sudah selesai Seminar Proposal</span>
                                                @elseif($pengajuan->mahasiswa->user->sempro)
                                                <span class="badge bg-primary" style="font-size: 10pt;">Sudah mendaftar Seminar Proposal</span>
                                                @else
                                                <span class="badge bg-secondary" style="font-size: 10pt;">Belum mendaftar Seminar Proposal</span>
                                                @endif
                                            </td>
                                            <td class="text-center" wire:click="openDetail('{{ $pengajuan->id }}')">
                                                @if(($pengajuan->pembimbing_1 === auth()->id() && $pengajuan->approve_pembimbing1) || ($pengajuan->pembimbing_2 === auth()->id() && $pengajuan->approve_pembimbing2))
                                                <span><i class="fas fa-check text-success"></i></span>
                                                @else
                                                <button class="btn btn-success btn-sm" wire:click="openModal('{{ $pengajuan->mahasiswa->id }}')">Setujui</button>
                                                @endif
                                            </td>
                                            <td class="text-center" wire:click="openDetail('{{ $pengajuan->id }}')">
                                                @if($pengajuan->jadwal && ($pengajuan->pembimbing_1 === auth()->id()))
                                                    @if(!$pengajuan->mahasiswa->user->sempro->revisi_pembimbing_1)
                                                        <button class="btn btn-success btn-sm" wire:click="openRevisiModal('{{ $pengajuan->mahasiswa->id }}')">Setujui</button>
                                                    @else
                                                    <span><i class="fas fa-check text-success"></i></span>
                                                    @endif
                                                @elseif($pengajuan->jadwal && ($pengajuan->pembimbing_2 === auth()->id()))
                                                    @if(!$pengajuan->mahasiswa->user->sempro->revisi_pembimbing_2)
                                                        <button class="btn btn-success btn-sm" wire:click="openRevisiModal('{{ $pengajuan->mahasiswa->id }}')">Setujui</button>
                                                    @else
                                                    <span><i class="fas fa-check text-success"></i></span>
                                                    @endif
                                            ...
                                            <td class="text-center">
                                                @if($pengajuan->jadwal && ($pengajuan->pembimbing_1 === auth()->id()))
                                                    ...
                                                    @endphp
                                                    <span style="display: flex;">{{ number_format(($total1 * 50 / 100) + ($total2 * 50 / 100), 2) }} <a href="javascript:;" style="margin-left: 5px;" wire:click="openEditNilaiModal('{{ $pengajuan->mahasiswa->user->sempro->id }}')"><i class="fa fa-edit text-secondary"></i></a></span>
                                                    @else
